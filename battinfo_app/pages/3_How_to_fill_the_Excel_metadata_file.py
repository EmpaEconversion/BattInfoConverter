...
- To fill in the metadata using our template file, simply fill in the data in the `@Schema` tab. Additional ontology terms can be added in the @Predicates and `@Classes` tab as needed.
...
### @Classes Tab
All unique string values (not numbers) are highly recommended to come with their own persistent online unique resource identifier (online persistent unique identifier).
...
- Some items are ontologized but lack a unique ID. For example, "R2032," which is the format of a coin cell case, is listed in the BattInfo ontology but does not have a unique ID.
